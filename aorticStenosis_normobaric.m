...
Rs = 15.1125;                      %Systemic resistance: mmHg/(L/min) %Original: 17.86
Rmi = .01;                       %mitral valve resistance: mmHg/(L/min)
RAo = 1;                       %Aortic valve resistance: mmHg/(L/min) %Original: .01

Vlvd = .027;                     %Left ventricular volume when PLV=0: L
Vsad = .825;                     %Systemic arterial volume when Psa+0: L
Pla = 5;                         %Left atrial pressure: mmHg %Original: 5

%Parameters for Clv(t)
CLVD = .0146;                   %Max (diastolic) value of CLV: L/mmHg
CLVS = .00003;                  %Min (systolic) value of CLV: L/mmHg %Original: .00003
